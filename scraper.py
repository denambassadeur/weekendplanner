...
def ask_gemini(promo_data):
    ...
    try:
        # Hier gebruiken we de variabele 'INSTRUCTIES'
        response = client.models.generate_content(
            model="gemini-2.0-flash",
            contents=f"Hier zijn de promoties: {promo_data}. Maak mijn plan.",
            config={
                'system_instruction': INSTRUCTIES
            }
        )
        return response.text
    except Exception as e:
        return f"Gemini kon niet antwoorden: {str(e)}"
...
if __name__ == "__main__":
    ...
    week_p = scrape_products(urls.get("week"))
    weekend_p = scrape_products(urls.get("weekend"))
    totaal_p = week_p + "\n" + weekend_p
    
    if len(totaal_p.strip()) > 10:
        inhoud = ask_gemini(totaal_p)
    else:
        inhoud = "Kon geen promoties ophalen van Lidl. De website blokkeert de toegang."
    ...
    <head>
        ...
        <style>
            body {{ font-family: sans-serif; max-width: 800px; margin: 40px auto; padding: 20px; line-height: 1.6; background-color: #f0f2f5; }}
            .card {{ border: 1px solid #ddd; padding: 30px; border-radius: 12px; background-color: #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }}
